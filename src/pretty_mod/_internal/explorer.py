...
class _ModuleTreeExplorerImpl:
    ...
    def _get_module_public_api(
        ...
        try:
            if hasattr(module, "__all__") and hasattr(module.__all__, "__iter__"):
                all_items = list(getattr(module, "__all__", []))
                api["all"] = all_items
                items = {
...
def _display_signature_impl(import_path: str) -> str:
    """
...
def _display_tree_impl(root_module_path: str, max_depth: int = 2) -> None:
    """Display the module tree.

    Args:
        root_module_path: String representing the root module (e.g., 'json', 'prefect.runtime')
        max_depth: Maximum depth to explore in the module tree (default: 2)
    """
    print(_ModuleTreeExplorerImpl(root_module_path, max_depth).get_tree_string())
