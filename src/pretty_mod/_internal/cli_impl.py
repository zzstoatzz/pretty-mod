...
from .explorer import _display_signature_impl, _ModuleTreeExplorerImpl


def _tree_command(args) -> None:
    """Handle the tree subcommand."""
    explorer = _ModuleTreeExplorerImpl(args.module, max_depth=args.depth)
    print(explorer.get_tree_string())


def _sig_command(args) -> None:
    """Handle the sig subcommand."""
    result = _display_signature_impl(args.import_path)
    print(result)
...
Examples:
  ...
  pretty-mod sig os.path:join                # Show function signature
    ...
    tree_parser.set_defaults(func=_tree_command)
    ...
    sig_parser.set_defaults(func=_sig_command)
